...
class AutocompletePrompt extends Base {
	...
	render(error /*: ?string */) {
		...
		if (this.status === "answered") {
			...
			bottomContent +=
				"  " + pc.dim(this.opt.searchText || "Searching...");
			...
			content += this.rl.line;
			bottomContent +=
				"  " + pc.yellow(this.opt.emptyText || "No results...");
		}
	...
	search(searchTerm /* : ?string */) /*: Promise<any>*/ {
		...
		return thisPromise.then((choices) => {
			...
			const realChoices = choices.filter((choice) =>
				isSelectable(choice)
			);
			...
			const selectedIndex = realChoices.findIndex(
				(choice) =>
					choice === this.initialValue ||
					choice.value === this.initialValue
			);
